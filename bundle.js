...
/***/ function(module, exports, __webpack_require__) {
	...
	var smallFunc = __webpack_require__(8);

	var setMoveFunc = smallFunc.setMove;
	var checkWin = smallFunc.check;
	var notEmpty = smallFunc.notEmptyElement;
	var setRand = smallFunc.setRandom;
	...
	var arr = ['', '', '', '', '', '', '', '', ''];
	var player_x_count = $('#Player_x-score').text();
	var player_o_count = $('#Player_o-score').text();
	var tie_count = $('#tie-score').text();
	var showAndSetMove = function showAndSetMove(event) {
	  event.preventDefault();
	  var id = "#" + event.target.id;
	  var id_num = parseInt(id.slice(-1)) - 1;
	  if ($(id).text() === "x" || $(id).text() === "o") {
	    ...
	    api.updateGameStatus(id_num, arr[id_num], over).done(ui.success).fail(ui.failure);
	  }
	};
	...
	var player_x_counts = $('#Player_x-score').text();
	var player_o_counts = $('#Player_o-score').text();
	var tie_counts = $('#tie-score').text();
	var showAndSetMove2 = function showAndSetMove2(event) {
	  event.preventDefault();
	  var id = "#" + event.target.id;
	  var id_num = parseInt(id.slice(-1)) - 1;
	  if ($(id).text() === "x" || $(id).text() === "o") {
	    ...
	    for (var prop in neighbour) {
	      if (parseInt(prop) === id_num) {
	        var nbArr = neighbour[prop];
	    ...
	    api.updateGameStatus(id_num, array[id_num], over).done(ui.success).fail(ui.failure);
	  }
	};

	var startNew = function startNew(event) {
	  event.preventDefault();
	...
	var getGame = function getGame(event) {
	  event.preventDefault();
	  //let id = $("#game-id").val();
	  api.getGameApi().done(ui.getGameSuccess).fail(ui.failure);
	};
...
/***/ function(module, exports, __webpack_require__) {
	...
	var getGameApi = function getGameApi() {
	  // if (id === '') {
	  return $.ajax({
	    url: app.host + '/games',
	    method: 'GET',
	...
	var updateGameStatus = function updateGameStatus(gameIndex, gameValue, gameStatus) {
	  return $.ajax({
...
/***/ function(module, exports, __webpack_require__) {
	...
	var app = __webpack_require__(6);
	var smallFunc = __webpack_require__(8);
	var signInApi = __webpack_require__(9);
	...
	var successSignUp = function successSignUp(data) {
	  ...
	  var ps = $('#sign-up-pw').val();
	  signInApi.signIn(email, ps).done(signInSuccess).fail(failure);
	  $('#sign-up-email').val('');
	  $('#sign-up-pw').val('');
	  $('#sign-up-pw-retype').val('');
	};
	...
	var getGameSuccess = function getGameSuccess(data) {
	  ...
	  var gameNumber = gameArr.length;
	  var x_score_count = 0;
	  var o_score_count = 0;
	  var tie_score_count = 0;
	...
	var returnGameId = function returnGameId() {
	  return gameId;
	};
...
/***/ function(module, exports) {
	...
	var notEmptyElement = function notEmptyElement(element) {
	  return element !== '';
	};

	var setRandom = function setRandom(bool) {
	  random = bool;
	};
	...
	module.exports = {
	  setMove: setMove,
	  check: check,
	  notEmptyElement: notEmptyElement,
	  setRandom: setRandom
	};
